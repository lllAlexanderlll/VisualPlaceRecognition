...
public abstract class AbstractSearchStructure {
	...
	public String getId(int iid) {
		if (iid < 0 || iid > loadCounter) {
			Log.i(TAG, "Internal id " + iid + " is out of range!");
			return null;
		}
		DatabaseEntry key = new DatabaseEntry();
		...
		if ((iidToIdDB.get(null, key, data, null) == OperationStatus.SUCCESS)) {
			...
			Log.i(TAG, "Internal id " + iid + " is in range but id was not found..");
			Log.i(TAG, "Index is probably corrupted");
			System.exit(0);
			return null;
		}
	}
	...
	public LatLng getGeolocation(int iid) {
		...
		if ((iidToGeolocationDB.get(null, key, data, null) == OperationStatus.SUCCESS)) {
			...
			Log.i(TAG, "Internal id " + iid + " is in range but gelocation was not found.");
			return null;
		}
	}
	...
	private void createOrOpenBDBDbs() throws Exception {
		...
		if (countSizeOnLoad) {
			Log.i(TAG, new Date() + " counting index size started ");
			int idToNameMappings = (int) iidToIdDB.count();
			loadCounter = Math.min(idToNameMappings, maxNumVectors);
			Log.i(TAG, new Date() + " counting index size ended ");
			Log.i(TAG, "Index size: " + loadCounter);
		}
	 ...
	 * @throws Exception
	 */
	private void createOrOpenBDBEnv(File BDBEnvHomeDir) throws Exception {
		// create the BDBEnvHome directory if it does not exist
		if (!BDBEnvHomeDir.isDirectory()) {
			boolean success = BDBEnvHomeDir.mkdir();
			if (success) {
				Log.i(TAG, BDBEnvHomeDir.getAbsolutePath() + " directory created.");
			}
		} else {
			Log.i(TAG, BDBEnvHomeDir.getAbsolutePath() + " directory exists.");
		}
		...
		try {
			...
			Log.i(TAG, "An existing BDB environment was found.");
		} catch (EnvironmentNotFoundException e) {
			envConf.setAllowCreate(true);
			dbEnv = new Environment(BDBEnvHomeDir, envConf);
			Log.i(TAG, "A new BDB environment was created.");
		}

		// printing information about the BDB environment
		Log.i(TAG, "== BDB environment configuration ===");
		Log.i(TAG, dbEnv.getConfig().toString());
		Log.i(TAG, "== BDB environment database names ===");
	 ...
	 * @throws Exception
	 */
	protected void createOrOpenBDBEnvAndDbs(File BDBEnvHome) throws Exception {
	...
	public void outputIndexingTimes() {
		...
		Log.i(TAG, (double) totalIdMappingTime / loadCounter + " ms => id mapping time");
		Log.i(TAG, (double) totalVectorIndexingTime / loadCounter + " ms => total indexing time");
	...
	public void close() {
		if (dbEnv != null) {
			...
			Log.i(TAG, "BDB environment is null!");
		}
	}
