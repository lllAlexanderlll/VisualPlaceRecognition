...
public class PQ extends AbstractSearchStructure {
	 ...
	 * @throws Exception
	 */
	public PQ(int vectorLength, int maxNumVectors, boolean readOnly, File BDBEnvHome, int numSubVectors,
	 ...
	 * 
	 * @param file
	 *            file pointing to the product quantizer
	 * @throws Exception
	 */
	public void loadProductQuantizer(File file) throws Exception {
...
//					Log.i(TAG, String.format("Loaded: %s", line));
				}
			}
		}
		in.close();
	...
	private void loadIndexInMemory() throws Exception {
		...
		Log.i(TAG, counter + " vectors loaded in " + (end - start) + " ms!");
	}
